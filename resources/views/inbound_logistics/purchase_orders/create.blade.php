    ...
    <div class="section-card">
        <form action="{{ route('purchase-orders.store') }}" method="POST" style="padding:24px;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:24px;">
                ...
                <div>
                    <label style="display:block;font-size:.875rem;font-weight:600;color:#374151;margin-bottom:8px;">Payment Terms</label>
                    <select name="payment_terms" style="width:100%;padding:10px 12px;border:1px solid #D1D5DB;border-radius:8px;font-size:.9rem;" required>
                        <option value="">-- Select Payment Terms --</option>
                        <option value="pre" {{ old('payment_terms') === 'pre' ? 'selected' : '' }}>Pre-payment</option>
                        <option value="post" {{ old('payment_terms') === 'post' ? 'selected' : '' }}>Post-payment</option>
                    </select>
                </div>
            </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:24px;">
                <div>
                    <label style="display:block;font-size:.875rem;font-weight:600;color:#374151;margin-bottom:8px;">Issue Date</label>
                    <input type="date" name="issue_date" value="{{ old('issue_date', now()->toDateString()) }}" style="width:100%;padding:10px 12px;border:1px solid #D1D5DB;border-radius:8px;font-size:.9rem;" required>
                </div>
                <div>
                    <label style="display:block;font-size:.875rem;font-weight:600;color:#374151;margin-bottom:8px;">Due Date</label>
                    <input type="date" name="due_date" value="{{ old('due_date', now()->addDays(7)->toDateString()) }}" style="width:100%;padding:10px 12px;border:1px solid #D1D5DB;border-radius:8px;font-size:.9rem;" required>
                </div>
            </div>
                        ...
                        <div>
                            ...
                            <select name="items[0][product_id]" style="width:100%;padding:10px 12px;border:1px solid #D1D5DB;border-radius:8px;font-size:.9rem;" required>
                                ...
                                @foreach($products as $product)
                                    <option value="{{ $product->id }}" {{ old('items.0.product_id') == $product->id ? 'selected' : '' }}>{{ $product->sku }} – {{ $product->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            <label style="display:block;font-size:.875rem;font-weight:600;color:#374151;margin-bottom:6px;">Quantity</label>
                            <input type="number" name="items[0][quantity]" min="1" value="{{ old('items.0.quantity', 1) }}" style="width:100%;padding:10px 12px;border:1px solid #D1D5DB;border-radius:8px;font-size:.9rem;" required>
                        </div>
